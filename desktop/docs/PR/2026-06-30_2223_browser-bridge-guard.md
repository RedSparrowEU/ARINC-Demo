...
Prevent the desktop renderer from failing with a raw JavaScript property error when it is rendered without the Electron preload bridge.

## Context

The renderer currently assumes `window.aeronav` always exists. When the page is opened in a regular browser tab or the preload bridge is otherwise unavailable, package import fails with an undefined-property exception instead of an actionable message.
...
3. Add a focused test for missing-bridge detection and rerun desktop validation.
...
Revert the desktop bridge-guard commit to restore the previous direct `window.aeronav` calls.
